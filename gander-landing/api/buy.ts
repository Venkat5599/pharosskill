...
// OpenAI-compatible model instead of regex. Falls back to buildRequest() on any
// failure so the demo never breaks.
const TR_BASE = process.env.TOKENROUTER_BASE_URL ?? "https://api.tokenrouter.com/v1";
const TR_MODEL = process.env.TOKENROUTER_MODEL ?? "MiniMax-M3";
...
async function planRequest(message: string): Promise<SafeBuyRequest> {
  ...
  try {
    const r = await fetch(`${TR_BASE}/chat/completions`, {
      ...
      body: JSON.stringify({
        model: TR_MODEL, temperature: 0, max_tokens: 1024,
        messages: [{ role: "system", content: TR_SYSTEM }, { role: "user", content: message }],
      }),
    });
    if (!r.ok) throw new Error(`tokenrouter ${r.status}`);
    const j = (await r.json()) as { choices?: { message?: { content?: string } }[] };
    const raw = JSON.parse(extractJson(j.choices?.[0]?.message?.content ?? "")) as Partial<SafeBuyRequest> & { schema?: string };
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  res.json({ explorer: EXPLORER, brain: process.env.TOKENROUTER_API_KEY ? "tokenrouter" : "regex", ...result });
}
